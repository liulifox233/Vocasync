...
use crate::{config::Config, handler::*, vocasync::Vocasync};
...
mod tests {
    ...
    async fn test() {
        ...
        let app = Router::new()
            ...
            .route(
                "/test/get_music_by_playlist/:id",
                get(get_music_by_playlist_test),
            )
            .with_state(vocasync)
            .layer(session_layer);
        ...
        reqwest::get(format!("{}/test/add", config.listen_address.clone(),))
            .await
            .unwrap();
        ...
        reqwest::get(format!("{}/api/currentPlay", config.listen_address.clone(),))
            .await
            .unwrap();

        reqwest::get(format!("{}/api/playList", config.listen_address.clone(),))
            .await
            .unwrap();
    }
}
